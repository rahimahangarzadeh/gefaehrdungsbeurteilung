...
// ---- BEREICH 1 ----
  ...
  data.email               = document.getElementById("email").value.trim(); // NEU
...
const templates = {
  rohrbau: {
    ...
    sa3_hg_mat: "Leder",
    gef1: true,
    gef1_mass: "Absturzsicherung durch Geländer und PSA",
    gef2: true,
    gef2_mass: "Verbauarbeiten nach DIN 4124, regelmäßige Kontrolle",
    gef5: true,
    gef5_mass: "Einweisung in Maschinenbedienung, Sicherheitsabstände einhalten"
  },
  ...
  tiefbau: {
    ...
    sa3_hg_mat: "Leder",
    gef1: true,
    gef1_mass: "Absturzsicherungen an Baugruben, PSAgA",
    gef2: true,
    gef2_mass: "Verbauarbeiten, Böschungswinkel prüfen, keine Annäherung an Grabenkante",
    gef5: true,
    gef5_mass: "Einweisung Baumaschinen, Sicherheitsabstände",
    gef7: true,
    gef7_mass: "Absperrungen, Verkehrssicherung nach RSA, Warnkleidung"
  },
  ...
  bohrung: {
    ...
    sa3_hg_mat: "Nitril",
    gef2: true,
    gef2_mass: "Baugrundgutachten beachten, Bohrflüssigkeit kontrollieren",
    gef4: true,
    gef4_mass: "Bentonit: Hautschutz, Handschuhe tragen",
    gef5: true,
    gef5_mass: "Einweisung Bohrgerät, Not-Aus zugänglich, Quetschgefahr vermeiden"
  },
  ...
  lager: {
    ...
    sa3_hg_mat: "Textil",
    gef1: true,
    gef1_mass: "Regale sicher aufgestellt, Leitern nach TRBS 2121 Teil 2",
    gef5: true,
    gef5_mass: "Staplerschein vorweisen, Fußgängerwege markiert, Spiegel an Ecken",
    gef7: true,
    gef7_mass: "Warnweste im gesamten Lager, Verkehrswege freihalten"
  },
  ...
  fernwaerme: {
    ...
    sa3_sb: true,
    sa3_hg: true,
    sa3_hg_mat: "Hitzebeständig",
    sa3_as: true,
    gef2: true,
    gef2_mass: "Verbauarbeiten, Leitungspläne vorhanden",
    gef3: true,
    gef3_mass: "Schweißerlaubnis, Feuerlöscher bereit, Umgebung prüfen",
    gef4: true,
    gef4_mass: "Dämmaterial: Atemschutz FFP2, Hautschutz",
    gef8: true,
    gef8_mass: "Heiße Leitungen: Schutzhandschuhe, Abkühlzeit beachten"
  }
};
...
function applyTemplate(template) {
  ...
  if (template.ausfuehrendeArbeiten) {
    document.getElementById('ausfuehrendeArbeiten').value = template.ausfuehrendeArbeiten;
  }
  
  // Organisation
  for (let i = 1; i <= 4; i++) {
    if (template[`org${i}`]) {
      const radio = document.querySelector(`input[name="org${i}"][value="${template[`org${i}`]}"]`);
      if (radio) radio.checked = true;
    }
  }
  
  // Sicheres Arbeiten - Speisepunkte
  ['sa1_bsv', 'sa1_esg', 'sa1_tt', 'sa1_ksv', 'sa1_prcd'].forEach(name => {
    const cb = document.querySelector(`input[name="${name}"]`);
    if (cb && template[name]) cb.checked = true;
  });
  
  // Sicheres Arbeiten - Sichtprüfung
  if (template.sa2) {
    const cb = document.querySelector('input[name="sa2"]');
    if (cb) cb.checked = true;
  }
  
  // Sicheres Arbeiten - PSA
  ['sa3', 'sa3_sh', 'sa3_ss', 'sa3_wv', 'sa3_gs', 'sa3_sb', 'sa3_ag', 'sa3_hg', 'sa3_as', 'sa3_so'].forEach(name => {
    const cb = document.querySelector(`input[name="${name}"]`);
    if (cb && template[name]) cb.checked = true;
  });
  
  if (template.sa3_hg_mat) {
    document.querySelector('input[name="sa3_hg_mat"]').value = template.sa3_hg_mat;
  }
  ...
  for (let i = 1; i <= 8; i++) {
    ...
    if (template[`gef${i}_mass`]) {
      const inp = document.querySelector(`input[name="gef${i}_mass"]`);
      if (inp) inp.value = template[`gef${i}_mass`];
    }
  }
}
